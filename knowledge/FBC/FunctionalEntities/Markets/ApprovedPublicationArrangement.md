---
type: FIBO Class
title: "approved publication arrangement"
description: "data reporting services provider that is authorized to provide the service of publishing certain trade reports on behalf of banks, investment firms, or asset management companies"
resource: https://spec.edmcouncil.org/fibo/ontology/FBC/FunctionalEntities/Markets/ApprovedPublicationArrangement
tags: [FBC, Release]
relations:
  - {type: is-a, target: "/FBC/FunctionalEntities/Markets/DataReportingServicesProvider.md", provenance: fibo}
  - {type: is-classified-by, target: "/FBC/FunctionalEntities/Markets/MarketCategoryClassifier-APPA.md", provenance: fibo}
---

data reporting services provider that is authorized to provide the service of publishing certain trade reports on behalf of banks, investment firms, or asset management companies

### Relationships
- is-a: [DataReportingServicesProvider](/FBC/FunctionalEntities/Markets/DataReportingServicesProvider.md)
- is-classified-by: [MarketCategoryClassifier-APPA](/FBC/FunctionalEntities/Markets/MarketCategoryClassifier-APPA.md)
