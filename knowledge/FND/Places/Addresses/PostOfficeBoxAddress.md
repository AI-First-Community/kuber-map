---
type: FIBO Class
title: "post office box address"
description: "physical address used to deliver communications to a designated container at a local post office or the local equivalent for the jurisdiction, where mail is held until the recipient collects it, rather than to a given house, building, or other location"
resource: https://spec.edmcouncil.org/fibo/ontology/FND/Places/Addresses/PostOfficeBoxAddress
tags: [FND, Release]
relations:
  - {type: comprises, target: "/FND/Places/Addresses/PostOfficeBox.md", provenance: fibo}
  - {type: is-a, target: "/FND/Places/Addresses/PhysicalAddress.md", provenance: fibo}
---

physical address used to deliver communications to a designated container at a local post office or the local equivalent for the jurisdiction, where mail is held until the recipient collects it, rather than to a given house, building, or other location

### Relationships
- comprises: [PostOfficeBox](/FND/Places/Addresses/PostOfficeBox.md)
- is-a: [PhysicalAddress](/FND/Places/Addresses/PhysicalAddress.md)
