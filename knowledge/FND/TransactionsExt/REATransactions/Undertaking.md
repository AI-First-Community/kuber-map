---
type: FIBO Class
title: "undertaking"
description: "Some undertaking to act."
resource: https://spec.edmcouncil.org/fibo/ontology/FND/TransactionsExt/REATransactions/Undertaking
tags: [FND, Informative]
relations:
  - {type: bestows, target: "/FND/Law/LegalCapacity/ContingentRight.md", provenance: fibo}
  - {type: gives-rise-to, target: "/FND/Agreements/Agreements/Commitment.md", provenance: fibo}
  - {type: has-contract-party, target: "/FND/Agreements/Contracts/ContractParty.md", provenance: fibo}
  - {type: is-made-as-part-of, target: "/FND/Agreements/Agreements/Agreement.md", provenance: fibo}
  - {type: is-undertaking-to, target: "/FND/Law/LegalCapacity/ContingentObligation.md", provenance: fibo}
---

Some undertaking to act.

### Relationships
- bestows: [ContingentRight](/FND/Law/LegalCapacity/ContingentRight.md)
- gives-rise-to: [Commitment](/FND/Agreements/Agreements/Commitment.md)
- has-contract-party: [ContractParty](/FND/Agreements/Contracts/ContractParty.md)
- is-made-as-part-of: [Agreement](/FND/Agreements/Agreements/Agreement.md)
- is-undertaking-to: [ContingentObligation](/FND/Law/LegalCapacity/ContingentObligation.md)
