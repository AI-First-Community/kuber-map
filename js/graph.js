...
(function () {
  ...
  // concepts across the loan-origination, KYC, securities, regulatory-reporting and derivatives use cases (the clean,
  ...
  function ucShort(u) {
    ...
    if (/^Regulatory/i.test(u)) return 'Regulatory reporting';
    if (/^Derivatives/i.test(u)) return 'Derivatives';
    return u.split(' (')[0];
  }
  ...
  const WELCOME_FEATURES = [
    { ic: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15 8.5 22 9.3 17 14 18.2 21 12 17.8 5.8 21 7 14 2 9.3 9 8.5 12 2"/></svg>',
      t: 'Curated core & lens', d: `You start on ${CORE_N} curated concepts spanning five use cases (loan origination, KYC, securities, regulatory reporting, derivatives). Use the Use-case lens to focus one; toggle Core → full ontology for all ${GRAPH.nodes.length}.` },
